...
`),l=new TextEncoder().encode(s);document.getElementById(`walletLoadingTitle`).innerText=`Signature Requested`,document.getElementById(`walletLoadingDesc`).innerText=`Approve the message in your wallet. This is a READ-ONLY signature — no SOL or tokens will be spent.`;let u=await r.signMessage(l,`utf8`),d=c(u.signature||u.signatureBytes);document.getElementById(`walletLoadingTitle`).innerText=`Verifying Authenticity...`,document.getElementById(`walletLoadingDesc`).innerText=`Validating cryptographic signature against Solana ledger standards on backend...`;let f=await fetch(`${e}/api/auth`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({address:i,message:s,signature:d})}),p=await f.text(),h;try{h=JSON.parse(p)}catch{throw console.error(`Failed to parse auth response as JSON. Raw text:`,p),Error(`Server returned non-JSON response (Status ${f.status}): ${p.substring(0,100)}...`)}if(h.success){t.wallet.connected=!0,t.wallet.provider=n,t.wallet.address=i,t.wallet.balanceSOL=h.user.balanceSOL||4.25,t.wallet.balanceEARN=h.user.balanceEARN||1250,t.userProfile.verified=h.user.verified||!1,t.userProfile.sorsaScore=h.user.sorsaScore||0;try{if(window.solanaWeb3){let e=await new solanaWeb3.Connection(solanaWeb3.clusterApiUrl(`devnet`),`confirmed`).getBalance(new solanaWeb3.PublicKey(i));t.wallet.balanceSOL=e/solanaWeb3.LAMPORTS_PER_SOL}}catch(e){console.log(`Real Solana ledger balance query bypassed. Using fallback balance:`,e)}document.getElementById(`walletText`).innerText=`${i.slice(0,4)}...${i.slice(-4)}`,document.getElementById(`walletBadgeDot`).className=`badge-dot`,m(),document.getElementById(`userBalanceDisplay`).style.display=`flex`,document.getElementById(`profileTogglesBadge`).style.display=`flex`,document.getElementById(`chkSimVerified`).checked=t.userProfile.verified,document.getElementById(`chkSimSorsa`).checked=t.userProfile.sorsaScore>0,o(),w(`Wallet cryptographically authenticated via ${n}!`,`success`),typeof fetchTasks==`function`?fetchTasks():typeof renderTaskList==`function`&&renderTaskList(),typeof renderMyCampaigns==`function`&&renderMyCampaigns(),typeof N==`function`&&N()}else throw Error(h.error||`Authentication rejected`)}catch(e){console.error(`Cryptographic signing error:`,e),w(`Verification Failed: ${e.message||e}`,`error`),o()}else w(`${n} extension not detected. Connecting in preview Demo Mode...`,`warning`),setTimeout(()=>{t.wallet.connected=!0,t.wallet.provider=`${n} (Demo)`;let r=Array.from({length:8},()=>Math.floor(Math.random()*16).toString(16)).join(``);t.wallet.address=`om8R${r.toUpperCase()}4s9`,fetch(`${e}/api/auth`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({address:t.wallet.address,message:`DEMO_BYPASS`,signature:`DEMO_BYPASS`})}).then(e=>e.json()).then(e=>{e.success&&(t.wallet.balanceSOL=4.25,t.wallet.balanceEARN=e.user?e.user.balanceEARN:1250,t.userProfile.verified=e.user?e.user.verified:!1,t.userProfile.sorsaScore=e.user?e.user.sorsaScore:0,document.getElementById(`walletText`).innerText=`${t.wallet.address.slice(0,4)}...${t.wallet.address.slice(-4)}`,document.getElementById(`walletBadgeDot`).className=`badge-dot`,m(),document.getElementById(`userBalanceDisplay`).style.display=`flex`,document.getElementById(`profileTogglesBadge`).style.display=`flex`,document.getElementById(`chkSimVerified`).checked=t.userProfile.verified,document.getElementById(`chkSimSorsa`).checked=t.userProfile.sorsaScore>0,o(),w(`Demo Wallet synchronized with server! Ready for review.`,`success`),typeof fetchTasks==`function`?fetchTasks():typeof renderTaskList==`function`&&renderTaskList(),typeof renderMyCampaigns==`function`&&renderMyCampaigns(),typeof N==`function`&&N())})},1500)}function u(){document.getElementById(`walletModalSelector`).style.display=`none`,document.getElementById(`walletModalTxSignIn`).style.display=`flex`,f(),lucide.createIcons()}function d(){document.getElementById(`walletModalTxSignIn`).style.display=`none`,document.getElementById(`walletModalSelector`).style.display=`flex`}function f(){let e=document.getElementById(`simTxWalletAddress`),t=document.getElementById(`btnTxStart`);!e||!t||(e.value.trim()===``?(t.setAttribute(`disabled`,`true`),t.style.opacity=`0.5`,t.style.cursor=`not-allowed`):(t.removeAttribute(`disabled`),t.style.opacity=`1`,t.style.cursor=`pointer`))}function p(){let n=document.getElementById(`simTxWalletAddress`).value.trim();if(!n){w(`Please enter a valid Solana wallet address.`,`error`);return}document.getElementById(`walletModalTxSignIn`).style.display=`none`;let r=document.getElementById(`walletModalLoading`);r.style.display=`flex`,document.getElementById(`walletLoadingTitle`).innerText=`Signing In with Transaction...`,document.getElementById(`walletLoadingDesc`).innerText=`Validating address registry on backend database...`,setTimeout(()=>{t.wallet.connected=!0,t.wallet.provider=`Transaction Auth`,t.wallet.address=n,fetch(`${e}/api/auth`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({address:t.wallet.address,message:`DEMO_BYPASS`,signature:`DEMO_BYPASS`})}).then(e=>e.json()).then(e=>{e.success&&(t.wallet.balanceSOL=4.25,t.wallet.balanceEARN=e.user?e.user.balanceEARN:1250,t.userProfile.verified=e.user?e.user.verified:!1,t.userProfile.sorsaScore=e.user?e.user.sorsaScore:0,document.getElementById(`walletText`).innerText=`${t.wallet.address.slice(0,4)}...${t.wallet.address.slice(-4)}`,document.getElementById(`walletBadgeDot`).className=`badge-dot`,m(),document.getElementById(`userBalanceDisplay`).style.display=`flex`,document.getElementById(`profileTogglesBadge`).style.display=`flex`,document.getElementById(`chkSimVerified`).checked=t.userProfile.verified,document.getElementById(`chkSimSorsa`).checked=t.userProfile.sorsaScore>0,o(),w(`Wallet connected successfully via transaction address!`,`success`),typeof fetchTasks==`function`?fetchTasks():typeof renderTaskList==`function`&&renderTaskList(),typeof renderMyCampaigns==`function`&&renderMyCampaigns(),typeof N==`function`&&N())})},2e3)}function m(){document.getElementById(`balanceText`).innerText=`${t.wallet.balanceEARN.toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})} $EARN`}function h(){let e=document.getElementById(`chkSimVerified`);t.userProfile.verified=e.checked,t.userProfile.verified?w(`Blue Tick simulation activated! You can now verify verified-only tasks.`,`success`):w(`Blue Tick simulation deactivated.`,`warning`),typeof renderTaskList==`function`&&renderTaskList()}function g(){let e=document.getElementById(`chkSimSorsa`);t.userProfile.sorsaScore=e.checked?10:0,t.userProfile.sorsaScore>0?w(`Sorsa Score set to > 0! You can now verify trust-gated tasks.`,`success`):w(`Sorsa Score reset.`,`warning`),typeof renderTaskList==`function`&&renderTaskList()}function _(){if(t.social.connected){t.social.connected=!1,t.social.handle=null,t.social.displayName=null,t.social.avatar=null,document.getElementById(`socialText`).innerText=`Connect X Account`,w(`X (Twitter) account disconnected.`,`warning`),typeof renderTaskList==`function`&&renderTaskList();return}document.getElementById(`modalConnectSocial`).classList.add(`active`),document.getElementById(`socialModalForm`).style.display=`flex`,document.getElementById(`socialModalLoading`).style.display=`none`}function v(){document.getElementById(`modalConnectSocial`).classList.remove(`active`)}function y(){let e=document.getElementById(`socialXHandle`).value.trim();if(!e){w(`Please enter a valid X username.`,`error`);return}document.getElementById(`socialModalForm`).style.display=`none`,document.getElementById(`socialModalLoading`).style.display=`flex`,setTimeout(()=>{t.social.connected=!0,t.social.handle=e,t.social.displayName=e.charAt(0).toUpperCase()+e.slice(1),t.social.avatar=`https://api.dicebear.com/7.x/identicon/svg?seed=${e}`,document.getElementById(`socialText`).innerText=`@${e}`,v(),w(`@${e} Twitter account linked successfully!`,`success`),typeof renderTaskList==`function`&&renderTaskList()},1500)}function b(){document.getElementById(`statTotalPaid`).innerText=`${t.stats.totalPaid.toLocaleString()} $EARN`,document.getElementById(`statCompleted`).innerText=`${t.stats.completedTasks.toLocaleString()} Tasks`,document.getElementById(`statActiveUsers`).innerText=`${t.stats.activeUsers.toLocaleString()} Users`,document.getElementById(`statTokenPrice`).innerText=`${t.stats.tokenPriceSOL.toFixed(6)} SOL`,document.getElementById(`statPriceChange`).innerText=`▲ %${t.stats.priceChange24h.toFixed(2)}`}function x(){let e=document.getElementById(`workersLeaderboardBody`),n=document.getElementById(`creatorsLeaderboardBody`);e&&(e.innerHTML=``,t.leaderboards.workers.forEach(t=>{let n=document.createElement(`tr`);n.className=`leaderboard-row`,n.innerHTML=`
    ...
    `,r.querySelector(`.toast-text`).textContent=e,n.appendChild(r),lucide.createIcons(),setTimeout(()=>{r.style.opacity=`0`,r.style.transform=`translateY(20px)`,setTimeout(()=>{n.removeChild(r)},300)},4e3)}function T(e){let t=e.parentElement,n=t.classList.contains(`active`);t.parentElement.querySelectorAll(`.faq-item`).forEach(e=>{e.classList.remove(`active`)}),n||t.classList.add(`active`)}var E=null;function D(){let e=document.getElementById(`vaultCountdownText`),t=document.getElementById(`vaultNextDateText`);if(!e)return;let n=Date.now()+10*3600*1e3+3240*1e3+39*1e3,r=new Date(n),i=r.getDate()<10?`0`+r.getDate():r.getDate(),a=r.getMonth()+1<10?`0`+(r.getMonth()+1):r.getMonth()+1,o=r.getFullYear(),s=r.getHours()<10?`0`+r.getHours():r.getHours(),c=r.getMinutes()<10?`0`+r.getMinutes():r.getMinutes(),l=r.getSeconds()<10?`0`+r.getSeconds():r.getSeconds();t&&(t.innerText=`${a}/${i}/${o} ${s}:${c}:${l}`),E&&clearInterval(E),E=setInterval(()=>{let t=n-Date.now();if(t<=0){n=Date.now()+12*3600*1e3;return}let r=Math.floor(t/(3600*1e3)),i=Math.floor(t%(3600*1e3)/(60*1e3)),a=Math.floor(t%(60*1e3)/1e3);e.innerText=`${r<10?`0`+r:r}:${i<10?`0`+i:i}:${a<10?`0`+a:a}`},1e3)}function O(){let e=document.getElementById(`vaultReturnBarChart`);if(!e)return;e.innerHTML=``;let t=e.clientWidth||400,n=e.clientHeight||200,r=t-10-10,i=n-15-20,a=[.06,.07,.11,.09,.08,.15,.04,.05,.07,.11,.1,.08,.11,.04,.04,.12,.12,.1,.12,.07,.12,.19,.15,.09,.1,.14,.32,.28,.19,.19],o=r/a.length,s=o*.65;a.forEach((t,n)=>{let r=t/.35*i,a=10+n*o+(o-s)/2,c=15+i-r,l=document.createElementNS(`http://www.w3.org/2000/svg`,`rect`);l.setAttribute(`x`,a),l.setAttribute(`y`,c),l.setAttribute(`width`,s),l.setAttribute(`height`,r),l.setAttribute(`fill`,`var(--text-success)`),l.setAttribute(`rx`,`1`),l.addEventListener(`mouseenter`,()=>{l.setAttribute(`fill`,`var(--secondary)`)}),l.addEventListener(`mouseleave`,()=>{l.setAttribute(`fill`,`var(--text-success)`)}),e.appendChild(l)});let c=document.createElementNS(`http://www.w3.org/2000/svg`,`text`);c.setAttribute(`x`,15),c.setAttribute(`y`,n-4),c.setAttribute(`fill`,`var(--text-muted)`),c.setAttribute(`font-size`,`0.65rem`),c.setAttribute(`font-weight`,`600`),c.textContent=`3 May`,e.appendChild(c);let l=document.createElementNS(`http://www.w3.org/2000/svg`,`text`);l.setAttribute(`x`,t-10-35),l.setAttribute(`y`,n-4),l.setAttribute(`fill`,`var(--text-muted)`),l.setAttribute(`font-size`,`0.65rem`),l.setAttribute(`font-weight`,`600`),l.textContent=`1 Jun`,e.appendChild(l)}var k=[{wallet:`Gmsr...wCY`,amount:`116.20 M`,eligible:`LP`,share:`36.9875%`},{wallet:`757k...6MR`,amount:`44.00 M`,eligible:`NO`,share:`0.0000%`},{wallet:`AUVA...2LG`,amount:`35.10 M`,eligible:`YES`,share:`36.9875%`},{wallet:`HF2L...AT3`,amount:`33.30 M`,eligible:`NO`,share:`0.0000%`},{wallet:`6meK...HJB`,amount:`28.40 M`,eligible:`NO`,share:`0.0000%`}];function A(){let e=document.getElementById(`vaultPreviewDistributionBody`);e&&(e.innerHTML=``,k.forEach(t=>{let n=document.createElement(`tr`);n.className=`leaderboard-row`;let r=``;r=t.eligible===`LP`?`<span class="vault-badge-LP">LP</span> <span class="vault-badge-no" style="margin-left: 2px;">NO</span>`:t.eligible===`YES`?`<span class="vault-badge-yes">YES</span>`:`<span class="vault-badge-no">NO</span>`,n.innerHTML=`
        ...
        `,lucide.createIcons();return}r.forEach(t=>{let r=e.wallet.connected&&t.completedBy.includes(e.wallet.address),i=document.createElement(`div`);i.className=`glass-card task-card`,r&&(i.style.opacity=`0.6`);let a=``;t.verifiedOnly?a=`<span style="font-size: 0.7rem; background: rgba(29,161,242,0.12); color:#1da1f2; border:1px solid rgba(29,161,242,0.25); padding: 0.15rem 0.4rem; border-radius: 4px; font-weight:700;"><i data-lucide="badge-check" style="width: 10px; height:10px; display:inline-block; vertical-align:middle; margin-right:2px;"></i>Verified Required</span>`:t.minSorsa>0&&(a=`<span style="font-size: 0.7rem; background: rgba(168,85,247,0.12); color:var(--primary); border:1px solid rgba(168,85,247,0.25); padding: 0.15rem 0.4rem; border-radius: 4px; font-weight:700;"><i data-lucide="shield-check" style="width: 10px; height:10px; display:inline-block; vertical-align:middle; margin-right:2px;"></i>Sorsa > 0 Required</span>`);let s=``;s=e.wallet.connected?e.social.connected?r?`
            ...
            `:`<button class="btn btn-primary" style="font-size: 0.8rem; padding: 0.45rem 0.9rem;" onclick="openTaskDetail('${t.id}')">View Task</button>`:`<button class="btn btn-outline" style="font-size: 0.8rem; padding: 0.45rem 0.9rem; border-color: rgba(29, 161, 242, 0.4); color: #1da1f2;" onclick="openConnectSocialModal()">Connect X Account</button>`:`<button class="btn btn-outline" style="font-size: 0.8rem; padding: 0.45rem 0.9rem;" onclick="openConnectWalletModal()">Connect Wallet</button>`,i.innerHTML=`
            <div class="task-card-header">
                ...
                <div style="display:flex; gap:0.4rem; align-items:center;">
                    ${a}
                    <span class="task-badge ${t.type}">${o(t.type)}</span>
                </div>
            </div>
            ...
            <div class="task-card-bottom">
                ...
                <div style="display: flex; flex-direction: column; align-items: flex-end; gap: 0.4rem;">
                    <div class="task-capacity">Entries: <span>${t.completedCount}/${t.capacity}</span></div>
                    ${s}
                </div>
            </div>
        `,n.appendChild(i)}),lucide.createIcons()}function o(e){switch(e){case`follow`:return`Follow`;case`like`:return`Like`;case`repost`:return`Repost`;case`feedback`:return`Review`;default:return`Task`}}function s(e,t){i=e,document.querySelectorAll(`.filter-btn`).forEach(e=>{e.classList.remove(`active`)}),t.classList.add(`active`),a()}var c=null,l=!1,u=`oauth`,d=``,f=null;function p(){f&&clearInterval(f),document.getElementById(`modalTaskDetail`).classList.remove(`active`)}function m(){let e=document.getElementById(`wurkSubmissionsList`);if(!e)return;let n=t.find(e=>e.id===c);n&&(e.style.display===`none`||e.style.display===``?(e.style.display=`block`,e.innerHTML=`
        ...
        `,lucide.createIcons()):e.style.display=`none`)}function h(e){window.open(e,`_blank`),l=!0;let t=document.getElementById(`verificationLockedState`);t&&(t.style.display=`none`);let n=document.getElementById(`verificationActiveState`);n&&(n.style.display=`flex`),_(u);let r=document.getElementById(`btnApplyAction`),i=document.getElementById(`btnVerifyTask`);r&&(r.style.display=`none`),i&&(i.style.display=`inline-flex`),showToast(`Task page opened! Please complete the interaction and fill out the proof form.`,`success`)}function g(e){c=e,l=!1,u=`oauth`,d=`EARN-${Array.from({length:4},()=>Math.floor(Math.random()*16).toString(16).toUpperCase()).join(``)}-${Date.now().toString().slice(-4)}`;let n=t.find(t=>t.id===e);if(!n)return;let r=n.creator.includes(`...`)?n.creator:`DaqHA9${n.creator.replace(/[^a-zA-Z0-9]/g,``).slice(0,6)}...VPrc`;f&&clearInterval(f);let i=Math.floor(Math.random()*5)+18,a=Math.floor(Math.random()*60),o=Math.floor(Math.random()*60);f=setInterval(()=>{o>0?o--:a>0?(a--,o=59):i>0?(i--,a=59,o=59):clearInterval(f);let e=i<10?`0`+i:i,t=a<10?`0`+a:a,n=o<10?`0`+o:o,r=document.getElementById(`closesInTimerDisplay`);r&&(r.innerText=`${e}h ${t}m ${n}s`)},1e3);let s=document.getElementById(`modalTaskDetail`),p=document.getElementById(`taskDetailContent`),m=(n.reward*2e-4).toFixed(2);p.innerHTML=`
    ...
    `,s.classList.add(`active`),lucide.createIcons()}function _(e){u=e;let t=document.getElementById(`proof-oauth`),n=document.getElementById(`proof-manual`),r=document.getElementById(`btnVerifyTask`),i=document.getElementById(`proofFlowDynamicArea`);e===`oauth`?(t.className=`tab-btn buy active`,n.className=`tab-btn buy`,i.innerHTML=`
            <div class="verify-steps" id="oauthProofSteps">
                <div class="verify-step ${l?`done`:`active`}" id="verifyStep1">
                    <span class="verify-step-check">${l?`✓`:`1`}</span>
                    <div>
                ...
                <div class="verify-step ${l?`active`:``}" id="verifyStep2">
                    <span class="verify-step-check">2</span>
                    <div>
                        <div style="font-weight: 700;">Verify Interaction</div>
                        <div style="font-size: 0.75rem; color: var(--text-muted);">Launch verification once you complete the task.</div>
            ...
            <div style="display:flex; flex-direction:column; gap:1rem;">
                <div style="background: rgba(255,255,255,0.02); border:1px solid var(--border-color); padding:1rem; border-radius:12px; display:flex; flex-direction:column; gap:0.5rem;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(0,0,0,0.2); border:1px solid rgba(255,255,255,0.05); padding:0.6rem 1rem; border-radius:8px;">
                        <code style="font-weight:800; color:var(--secondary); font-size:1.05rem;">I am verifying my X profile on earn.cool with code ${d}</code>
                        <button class="btn btn-outline" style="padding:0.25rem 0.5rem; font-size:0.7rem; border-radius:4px;" onclick="navigator.clipboard.writeText('I am verifying my X profile on earn.cool with code ${d}'); showToast('Verification template copied! Share it on X.', 'success')">Copy</button>
                    </div>
                </div>
        ...
        `),e===`oauth`?l?(r.removeAttribute(`disabled`),r.style.opacity=`1`,r.style.cursor=`pointer`):(r.setAttribute(`disabled`,`true`),r.style.opacity=`0.5`,r.style.cursor=`not-allowed`):v(),lucide.createIcons()}function v(){let e=document.getElementById(`manualProofTweetUrl`),t=document.getElementById(`btnVerifyTask`);if(!e||!t)return;let n=e.value.trim();n.startsWith(`https://x.com/`)&&n.includes(`/status/`)?(t.removeAttribute(`disabled`),t.style.opacity=`1`,t.style.cursor=`pointer`):(t.setAttribute(`disabled`,`true`),t.style.opacity=`0.5`,t.style.cursor=`not-allowed`)}async function y(){if(!c)return;let r=t.find(e=>e.id===c);if(!r)return;let i=document.getElementById(`verificationInnerContent`);if(!i)return;i.innerHTML=`
        <div class="verify-flow-card" style="padding: 1rem 0;">
            ...
            <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--border-color); padding: 0.75rem 1rem; border-radius: 10px; width: 100%; text-align: left; display: flex; flex-direction: column; gap: 0.4rem; font-size: 0.8rem; color: var(--text-muted);">
                ...
                <div class="verify-step" id="subStep2">
                    <span class="verify-step-check">•</span> ${u===`oauth`?`Requesting Twitter OAuth 2.0 API interaction proof...`:`Querying submitted Tweet verification code...`}
                </div>
    ...
    `;let o=document.getElementById(`btnVerifyTask`);o&&(o.style.display=`none`);try{await new Promise(e=>setTimeout(e,1e3)),document.getElementById(`subStep1`).className=`verify-step done`,document.getElementById(`subStep2`).className=`verify-step active`,document.getElementById(`verifyFlowSub`).innerText=`Accessing X network details to verify follow/like/repost actions...`,await new Promise(e=>setTimeout(e,1200)),document.getElementById(`subStep2`).className=`verify-step done`,document.getElementById(`subStep3`).className=`verify-step active`,document.getElementById(`verifyFlowSub`).innerText=`Releasing escrow contract funds to recipient address...`;let t={walletAddress:e.wallet.address,verifiedSim:e.userProfile.verified,sorsaScoreSim:e.userProfile.sorsaScore,proofMethod:u,tweetUrl:u===`manual`?document.getElementById(`manualProofTweetUrl`).value.trim():``},o=await(await fetch(`${n}/api/tasks/${r.id}/verify`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)})).json();if(await new Promise(e=>setTimeout(e,1e3)),o.success){e.wallet.balanceEARN=o.user.balanceEARN,updateNavbarBalance();let t=o.commissionAdded||(r.reward*.02).toFixed(2);r.completedBy.push(e.wallet.address),r.completedCount=o.task.completedCount,typeof fetchVaultStatus==`function`&&fetchVaultStatus(),i.innerHTML=`
                <div class="verify-flow-card" style="gap: 1rem; padding: 1rem 0;">
                    ...
                    <div>
                        ...
                        <p style="font-size: 0.85rem; color: var(--text-muted); line-height: 1.5;">
                            Congratulations, your X social interaction was verified by the platform database! <b style="color: var(--text-success);">+${r.reward} $EARN</b> was successfully credited to your wallet, and <b style="color: var(--primary);">+${t} $EARN</b> was added to the Vault commission rewards.
                        </p>
                    </div>
                    <button class="btn btn-outline" style="width: 100%;" onclick="closeTaskDetailModal()">Close & Go to Market</button>
                </div>
            `;let n=document.getElementById(`capacityRatioDisplay`);n&&(n.innerText=`${r.completedCount} / ${r.capacity}`),b(),showToast(`Task verified! +${r.reward} $EARN added to balance.`,`success`),a()}else i.innerHTML=`
                <div class="verify-flow-card" style="gap: 1rem; padding: 1rem 0;">
            ...
            `,showToast(o.error,`error`)}catch(e){console.error(e),showToast(`Server communication error during verification.`,`error`),p()}}function b(){let e=document.body;for(let t=0;t<35;t++){let t=document.createElement(`div`);t.className=`particle`,t.style.left=`${Math.random()*100}vw`,t.style.top=`-20px`;let n=Math.random()*8+8;t.style.width=`${n}px`,t.style.height=`${n}px`;let r=Math.random()>.5;t.style.setProperty(`--secondary`,r?`#06b6d4`:`#a855f7`),t.style.boxShadow=`0 0 10px ${r?`#06b6d4`:`#a855f7`}`,t.style.animationDuration=`${Math.random()*2+1.5}s`,t.style.animationDelay=`${Math.random()*.5}s`,e.appendChild(t),setTimeout(()=>{e.removeChild(t)},3500)}}function x(){let e=document.getElementById(`taskType`).value,t=document.getElementById(`lblTaskTitle`),n=document.getElementById(`taskTitle`),r=document.getElementById(`lblTaskLink`),i=document.getElementById(`taskLink`);e===`follow`?(t.innerText=`X Account Title (To Follow)`,n.placeholder=`e.g. Solana earn.cool Official Account Follow`,r.innerText=`X Page Link (Profile URL)`,i.placeholder=`e.g. https://x.com/earndotcool`):e===`like`?(t.innerText=`Tweet Title to Display (To Like)`,n.placeholder=`e.g. Like the earn.cool Devs Major Partnership tweet`,r.innerText=`Tweet Link (URL)`,i.placeholder=`e.g. https://x.com/earndotcool/status/1234567890`):e===`repost`?(t.innerText=`Post Title to Retweet`,n.placeholder=`e.g. Repost and spread the $EARN launch post`,r.innerText=`Tweet Link (URL)`,i.placeholder=`e.g. https://x.com/earndotcool/status/987654321`):e===`feedback`&&(t.innerText=`Feedback Task Title`,n.placeholder=`e.g. Test the earn.cool website and write a review`,r.innerText=`Review Web Page URL`,i.placeholder=`e.g. https://earn.cool/feedback`)}function S(){let e=parseFloat(document.getElementById(`rewardPerUser`).value)||0,t=parseInt(document.getElementById(`maxParticipants`).value)||0,n=document.getElementById(`campaignQualityFilter`).value,r=e*t,i=r*.02,a=0,o=document.getElementById(`summaryQualityRow`),s=document.getElementById(`summaryQualityLabel`),c=document.getElementById(`summaryQualityFee`);n===`verified`?(a=r*.2,o&&(o.style.display=`flex`,s.innerText=`Verified Targeting (+20%):`,c.innerText=`${a.toLocaleString()} $EARN`)):n===`sorsa`?(a=r*.15,o&&(o.style.display=`flex`,s.innerText=`Sorsa Score Targeting (+15%):`,c.innerText=`${a.toLocaleString()} $EARN`)):o&&(o.style.display=`none`);let l=r+i+a;document.getElementById(`summaryTotalReward`).innerText=`${r.toLocaleString()} $EARN`,document.getElementById(`summaryFee`).innerText=`${i.toLocaleString()} $EARN`,document.getElementById(`summaryTotalCost`).innerText=`${l.toLocaleString()} $EARN`}async function C(t){if(t.preventDefault(),!e.wallet.connected){showToast(`Wallet disconnected. Please connect your Solana wallet to launch a campaign.`,`error`),openConnectWalletModal();return}let i=document.getElementById(`taskType`).value,a=document.getElementById(`taskTitle`).value.trim(),o=document.getElementById(`taskLink`).value.trim(),s=parseFloat(document.getElementById(`rewardPerUser`).value),c=parseInt(document.getElementById(`maxParticipants`).value),l=document.getElementById(`campaignQualityFilter`).value,u=s*c,d=u*.02,f=0;l===`verified`?f=u*.2:l===`sorsa`&&(f=u*.15);let p=u+d+f;if(e.wallet.balanceEARN<p){showToast(`Insufficient balance! Required: ${p.toLocaleString()} $EARN, Balance: ${e.wallet.balanceEARN.toLocaleString()} $EARN.`,`error`);return}let m=document.getElementById(`modalConnectWallet`),h=document.getElementById(`walletModalSelector`),g=document.getElementById(`walletModalLoading`);h.style.display=`none`,g.style.display=`flex`,g.querySelector(`h3`).innerText=`Approving Campaign Budget...`,g.querySelector(`p`).innerText=`Approve the wallet signature request to lock the campaign budget in escrow.`,m.classList.add(`active`);try{let t={creatorAddress:e.wallet.address,totalCostEarn:p,task:{type:i,title:a,link:o,reward:s,capacity:c,verifiedOnly:l===`verified`,minSorsa:+(l===`sorsa`)}},u=await(await fetch(`${n}/api/tasks/create`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)})).json();await new Promise(e=>setTimeout(e,1500)),m.classList.remove(`active`),u.success?(u.user&&u.user.balanceEARN!==void 0?e.wallet.balanceEARN=u.user.balanceEARN:e.wallet.balanceEARN-=p,updateNavbarBalance(),typeof r==`function`&&await r(),u.task&&w(u.task,p),document.getElementById(`createCampaignForm`).reset(),selectQualityFilter(`all`),x(),S(),showToast(`Congratulations! Campaign successfully created and launched in the Task Market.`,`success`)):showToast(u.error||`Failed to create campaign.`,`error`)}catch(e){console.error(e),m.classList.remove(`active`),showToast(`Server communication error during campaign creation.`,`error`)}}function w(e,t){let n=document.getElementById(`myCampaignsList`);if(!n)return;n.innerHTML.includes(`You have not published any campaigns yet`)&&(n.innerHTML=``);let r=document.createElement(`div`);r.className=`glass-card`,r.style.padding=`1.25rem`,r.style.background=`rgba(255, 255, 255, 0.01)`,r.style.border=`1px solid rgba(255, 255, 255, 0.05)`;let i=e.completedCount/e.capacity*100;r.innerHTML=`
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.75rem;">
            <div>
                <h4 style="font-size: 0.95rem; font-weight: 700; line-height: 1.3;">${e.title}</h4>
                <span style="font-size: 0.75rem; color: var(--text-muted);">Type: ${o(e.type)}</span>
            </div>
    ...
    `,n.insertBefore(r,n.firstChild)}window.filterTasks=s,window.closeTaskDetailModal=p,window.toggleWurkSubmissions=m,window.executeTaskAction=h,window.openTaskDetail=g,window.switchProofMethod=_,window.checkManualProofInput=v,window.verifyTaskCompletion=y,window.triggerTokenRainAnimation=b,window.updateFormPlaceholders=x,window.calculateCampaignCost=S,window.handleCreateCampaign=C,window.addCampaignToMyList=w})),r=e((()=>{var e=window.state,t=[{time:`12:00`,open:185e-6,high:195e-6,low:18e-5,close:192e-6},{time:`12:05`,open:192e-6,high:21e-5,low:19e-5,close:205e-6},{time:`12:10`,open:205e-6,high:208e-6,low:195e-6,close:198e-6},{time:`12:15`,open:198e-6,high:215e-6,low:196e-6,close:212e-6},{time:`12:20`,open:212e-6,high:225e-6,low:21e-5,close:22e-5},{time:`12:25`,open:22e-5,high:238e-6,low:218e-6,close:23e-5},{time:`12:30`,open:23e-5,high:248e-6,low:228e-6,close:242e-6}],n=[{user:`om8R...4s9`,type:`buy`,amountSOL:.5,amountCLICK:2066.12,age:`1m ago`},{user:`An3k...xP8`,type:`buy`,amountSOL:1.2,amountCLICK:4958.67,age:`3m ago`},{user:`8hN1...Kqy`,type:`sell`,amountSOL:.4,amountCLICK:1652.89,age:`5m ago`},{user:`Sol_Degen`,type:`buy`,amountSOL:3.5,amountCLICK:14462.8,age:`8m ago`},{user:`J9u1...45s`,type:`buy`,amountSOL:.2,amountCLICK:826.44,age:`12m ago`}],r=`buy`;document.addEventListener(`DOMContentLoaded`,()=>{l()});function i(e){r=e;let t=document.getElementById(`tabBuy`),n=document.getElementById(`tabSell`),i=document.getElementById(`tradeInputLabel`),a=document.getElementById(`tradeAmountCurrency`),s=document.getElementById(`btnExecuteTrade`),c=document.getElementById(`tradeAmount`),l=document.getElementById(`buyQuickAmts`),u=document.getElementById(`sellQuickAmts`);e===`buy`?(t.className=`tab-btn buy active`,n.className=`tab-btn sell`,i.innerText=`Amount to Pay (SOL)`,a.innerText=`SOL`,s.innerText=`Buy Now (Buy)`,s.style.background=`linear-gradient(135deg, var(--text-success), #059669)`,s.style.boxShadow=`0 4px 15px rgba(16, 185, 129, 0.3)`,c.value=`0.5`,l.style.display=`grid`,u.style.display=`none`):(t.className=`tab-btn buy`,n.className=`tab-btn sell active`,i.innerText=`Amount to Sell ($EARN)`,a.innerText=`$EARN`,s.innerText=`Sell Now`,s.style.background=`linear-gradient(135deg, var(--text-error), #dc2626)`,s.style.boxShadow=`0 4px 15px rgba(239, 68, 68, 0.3)`,c.value=`500`,l.style.display=`none`,u.style.display=`grid`),o()}function a(t){let n=document.getElementById(`tradeAmount`);if(r===`buy`)n.value=t.toString();else if(e.wallet.connected){let r=e.wallet.balanceEARN*(t/100);n.value=Math.floor(r).toString()}else showToast(`Please connect your wallet first.`,`error`),openConnectWalletModal();o()}function o(){let t=parseFloat(document.getElementById(`tradeAmount`).value)||0,n=document.getElementById(`tradeReceiveAmount`),i=document.getElementById(`tradePriceImpact`);if(t<=0){n.innerText=`0.00 ${r===`buy`?`$EARN`:`SOL`}`,i.innerText=`%0.00`;return}if(r===`buy`)n.innerText=`${(t/e.stats.tokenPriceSOL).toLocaleString(void 0,{minimumFractionDigits:2,maximumFractionDigits:2})} $EARN`,i.innerText=`%${(t/e.stats.raydiumThresholdSOL*100).toFixed(3)} (Price Will Increase)`,i.style.color=`var(--text-success)`;else{let r=t*e.stats.tokenPriceSOL;n.innerText=`${r.toLocaleString(void 0,{minimumFractionDigits:4,maximumFractionDigits:4})} SOL`,i.innerText=`-%${(r/e.stats.raydiumThresholdSOL*100).toFixed(3)} (Price Will Decrease)`,i.style.color=`var(--text-error)`}}function s(){let e=document.getElementById(`priceCandleChart`),n=document.getElementById(`chartContainer`);if(!e||!n)return;e.innerHTML=``;let r=n.clientWidth,i=n.clientHeight,a=r-15-75,o=i-25-30,s=-1/0,c=1/0;t.forEach(e=>{e.high>s&&(s=e.high),e.low<c&&(c=e.low)});let l=s-c;s+=l*.15,c-=l*.15,c<0&&(c=0);for(let t=0;t<5;t++){let n=t/4,r=25+o*(1-n),i=c+n*(s-c),l=document.createElementNS(`http://www.w3.org/2000/svg`,`line`);l.setAttribute(`x1`,15),l.setAttribute(`y1`,r),l.setAttribute(`x2`,15+a),l.setAttribute(`y2`,r),l.setAttribute(`stroke`,`rgba(255, 255, 255, 0.04)`),l.setAttribute(`stroke-dasharray`,`4, 4`),e.appendChild(l);let u=document.createElementNS(`http://www.w3.org/2000/svg`,`text`);u.setAttribute(`x`,15+a+8),u.setAttribute(`y`,r+4),u.setAttribute(`fill`,`var(--text-muted)`),u.setAttribute(`font-size`,`0.75rem`),u.setAttribute(`font-weight`,`600`),u.textContent=`${i.toFixed(6)} SOL`,e.appendChild(u)}let u=a/t.length,d=u*.65;t.forEach((t,n)=>{let r=15+n*u+u/2,a=25+o*(1-(t.high-c)/(s-c)),l=25+o*(1-(t.low-c)/(s-c)),f=25+o*(1-(t.open-c)/(s-c)),p=25+o*(1-(t.close-c)/(s-c)),m=t.close>=t.open?`var(--text-success)`:`var(--text-error)`,h=document.createElementNS(`http://www.w3.org/2000/svg`,`line`);h.setAttribute(`x1`,r),h.setAttribute(`y1`,a),h.setAttribute(`x2`,r),h.setAttribute(`y2`,l),h.setAttribute(`stroke`,m),h.setAttribute(`stroke-width`,`1.5`),e.appendChild(h);let g=Math.min(f,p),_=Math.max(2,Math.abs(f-p)),v=document.createElementNS(`http://www.w3.org/2000/svg`,`rect`);if(v.setAttribute(`x`,r-d/2),v.setAttribute(`y`,g),v.setAttribute(`width`,d),v.setAttribute(`height`,_),v.setAttribute(`fill`,m),v.setAttribute(`rx`,`2`),e.appendChild(v),n%2==0){let n=document.createElementNS(`http://www.w3.org/2000/svg`,`text`);n.setAttribute(`x`,r),n.setAttribute(`y`,i-8),n.setAttribute(`fill`,`var(--text-muted)`),n.setAttribute(`font-size`,`0.7rem`),n.setAttribute(`text-anchor`,`middle`),n.setAttribute(`font-weight`,`600`),n.textContent=t.time,e.appendChild(n)}})}function c(){if(!e.wallet.connected){showToast(`Please connect wallet to perform trade.`,`error`),openConnectWalletModal();return}let i=parseFloat(document.getElementById(`tradeAmount`).value);if(!i||i<=0){showToast(`Please enter a valid trade amount.`,`error`);return}let a=document.getElementById(`modalConnectWallet`),c=document.getElementById(`walletModalSelector`),u=document.getElementById(`walletModalLoading`);if(c.style.display=`none`,u.style.display=`flex`,r===`buy`){if(e.wallet.balanceSOL<i){showToast(`Insufficient SOL balance! Required: ${i} SOL, Available: ${e.wallet.balanceSOL} SOL`,`error`);return}u.querySelector(`h3`).innerText=`Buying Tokens...`,u.querySelector(`p`).innerText=`Approve the wallet signature request to buy $EARN with ${i} SOL.`}else{if(e.wallet.balanceEARN<i){showToast(`Insufficient $EARN balance! Required: ${i.toLocaleString()} $EARN, Available: ${e.wallet.balanceEARN.toLocaleString()} $EARN`,`error`);return}u.querySelector(`h3`).innerText=`Selling Tokens...`,u.querySelector(`p`).innerText=`Approve the wallet signature request to sell ${i.toLocaleString()} $EARN.`}a.classList.add(`active`),setTimeout(()=>{a.classList.remove(`active`);let c=e.stats.tokenPriceSOL,u=0;if(r===`buy`){u=i/e.stats.tokenPriceSOL,e.wallet.balanceSOL-=i,e.wallet.balanceEARN+=u;let t=i*8e-6;e.stats.tokenPriceSOL+=t,e.stats.totalSOLInvested+=i,e.stats.bondingCurvePercent=Math.min(99.9,e.stats.totalSOLInvested/e.stats.raydiumThresholdSOL*100),n.unshift({user:e.wallet.address.slice(0,4)+`...`+e.wallet.address.slice(-4),type:`buy`,amountSOL:i,amountCLICK:u,age:`just now`}),triggerTokenRainAnimation(),showToast(`Success! +${u.toLocaleString(void 0,{maximumFractionDigits:2})} $EARN bought.`,`success`)}else{u=i*e.stats.tokenPriceSOL,e.wallet.balanceEARN-=i,e.wallet.balanceSOL+=u;let t=u*8e-6;e.stats.tokenPriceSOL=Math.max(1e-5,e.stats.tokenPriceSOL-t),e.stats.totalSOLInvested=Math.max(0,e.stats.totalSOLInvested-u),e.stats.bondingCurvePercent=Math.min(99.9,e.stats.totalSOLInvested/e.stats.raydiumThresholdSOL*100),n.unshift({user:e.wallet.address.slice(0,4)+`...`+e.wallet.address.slice(-4),type:`sell`,amountSOL:u,amountCLICK:i,age:`just now`}),showToast(`Success! +${u.toFixed(4)} SOL transferred to your wallet.`,`success`)}updateNavbarBalance(),renderStats(),l();let d=new Date,f=d.getMinutes()<10?`0`+d.getMinutes():d.getMinutes(),p=`${d.getHours()}:${f}`,m=c,h=e.stats.tokenPriceSOL,g=Math.max(m,h)+Math.random()*3e-6,_=Math.min(m,h)-Math.random()*3e-6;t.push({time:p,open:m,high:g,low:_,close:h}),t.length>8&&t.shift(),document.getElementById(`chartTokenPriceVal`).innerText=`${e.stats.tokenPriceSOL.toFixed(6)} SOL`,s(),document.getElementById(`tradeAmount`).value=r===`buy`?`0.5`:`500`,o()},1800)}function l(){let e=document.getElementById(`recentTokenTrades`);e&&(e.innerHTML=``,n.forEach(t=>{let n=document.createElement(`div`);n.style.display=`flex`,n.style.justifyContent=`space-between`,n.style.padding=`0.4rem 0.75rem`,n.style.background=`rgba(255, 255, 255, 0.01)`,n.style.border=`1px solid rgba(255, 255, 255, 0.03)`,n.style.borderRadius=`6px`;let r=t.type===`buy`;n.innerHTML=`
        ...
        `,e.appendChild(n)}))}window.switchTradeMode=i,window.setTradeAmount=a,window.calculateTradeReturn=o,window.initTradingChart=s,window.executeTokenTrade=c,window.renderRecentTrades=l}));t(),n(),r();